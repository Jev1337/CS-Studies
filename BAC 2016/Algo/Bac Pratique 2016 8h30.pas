...
Begin
  ...
  If (Premier(Fact(x) + 1)) Then
    // Si (factoriel de x) + 1 est un nombre premier alors le stocker dans Y et test:= true
    Begin
      foo := True;
      y := Fact(x) + 1
    End;
  If (Premier(Fact(x) - 1)) Then
    // Si (factoriel de x) - 1 est un nombre premier alors le stocker dans S et test:= true
    Begin
      foo := True;
      s := Fact(x) - 1;
    End;
...
Begin
  ...
  For i:= 1 To x Do
    If Premier(i) Then
      s := s*i;
  // Stocker la somme des nombres premiers inferieurs à x dans S
...
//Si (factoriel de x) + 1 est premier alors l'afficher et incrémenter j (et j <> n pour eviter le double incrementation)
          Begin
...
//Si (la somme des nombres premiers inferieurs à N) - 1 est premier alors l'afficher et incrémenter j
              Begin
...
//Si (la somme des nombres premiers inferieurs à N) + 1 est premier alors l'afficher et incrémenter j  (et j <> n pour eviter le double incrementation)
              Begin
